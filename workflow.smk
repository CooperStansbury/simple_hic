...
rule all:
    input:
        ...
        OUTPUT + 'references/gene.positions.csv',
        ...
        expand(f"{OUTPUT}pairs/{{fids}}.gene.pairs.csv", fids=fids),
...
rule get_gene_positions:
    input:
        gtf=config['annotations_path'],
    output:
        OUTPUT + 'references/gene.positions.csv',
    shell:
        """python scripts/getGenePositions.py {input.gtf} {output}"""
...
rule build_genexgene:
    input:
        pairs=OUTPUT + "pairs/{fid}.final.pairs.gz",
        genes=OUTPUT + 'references/gene.positions.csv',
    output:
        OUTPUT + "pairs/{fid}.gene.pairs.csv",
    params:
        slack=config['slack']
    wildcard_constraints:
        fids='|'.join([re.escape(x) for x in set(fids)]),
    shell:
        """python scripts/buildGxG.py {input.pairs} {input.genes} {params.slack} {output}"""
